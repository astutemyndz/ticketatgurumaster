...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjCountry&amp;action=pjActionCreate" method="post" id="frmCreateCountry" class="pj-form form">
		...
		<p>
			...
			<input type="button" value="<?php __('plugin_country_btn_cancel'); ?>" class="pj-button" onclick="window.location.href='<?php echo PJ_INSTALL_URL; ?>admin.php?controller=pjCountry&action=pjActionIndex';" />
		</p>
	</form>
