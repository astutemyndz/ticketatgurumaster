	...
	public function pjActionCreate()
	{
		...
		if ($this->isCountryReady())
		{
			if (isset($_POST['country_create']))
			{
				...
				pjUtil::redirect(PJ_INSTALL_URL . "admin.php?controller=pjCountry&action=pjActionIndex&err=$err");
			} else {
	...
	public function pjActionUpdate()
	{
		...
		if ($this->isCountryReady())
		{
			if (isset($_POST['country_update']))
			{
				...
				pjUtil::redirect(PJ_INSTALL_URL . "admin.php?controller=pjCountry&action=pjActionIndex&err=PCY01");
				
			} else {
				$arr = pjCountryModel::factory()->find($_GET['id'])->getData();
				if (count($arr) === 0)
				{
					pjUtil::redirect(PJ_INSTALL_URL. "admin.php?controller=pjCountry&action=pjActionIndex&err=PCY08");
				}
