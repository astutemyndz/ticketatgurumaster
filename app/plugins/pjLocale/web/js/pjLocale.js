	...
	$(function () {
		...
		if ($tabs.length > 0 && tabs) {
			$tabs.tabs({
				select: function (event, ui) {
					switch (ui.index) {
						case 0:
							window.location.href = 'admin.php?controller=pjAdminOptions&action=pjActionIndex&tab=0';
							break;
					}
				}
			});
	    	...
	    	$dialogFlagReset.dialog({
				...
				buttons: (function () {
					...
					buttons[myLabel.btn_reset] = function () {
						$.post('admin.php?controller=pjLocale&action=pjActionFlagReset', {
							'id': $dialogFlagReset.data('id')
						}).done(function (data) {
							if (data && data.status && data.status === 'OK') {
								var content = $grid.datagrid("option", "content");
								$grid.datagrid("load", "admin.php?controller=pjLocale&action=pjActionGetLocale", content.column, content.direction, content.page, content.rowCount);
							}
						}).always(function () {
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			if ($upload.length && zinoUpload) {
				$upload.zinoUpload({
					method: 'POST',
					url: 'admin.php?controller=pjLocale&action=pjActionUpload',
					name: 'flag',
			...
			var $grid = $("#grid").on('click', '.btn-locale-upload', function (e) {
				...
				buttons: [{type: "delete", url: "admin.php?controller=pjLocale&action=pjActionDeleteLocale&id={:id}", beforeShow: onBeforeShow}],
				...
				dataUrl: "admin.php?controller=pjLocale&action=pjActionGetLocale",
				dataType: "json",
				fields: ['language_iso', 'name', 'file', 'id', 'dir', 'is_default', 'sort'],
				paginator: false,
				saveUrl: "admin.php?controller=pjLocale&action=pjActionSaveLocale&id={:id}",
				sortable: true,
				sortableUrl: "admin.php?controller=pjLocale&action=pjActionSortLocale"
			});
			...
			$(document).on("click", ".btn-add", function (e) {
				...
				$.post("admin.php?controller=pjLocale&action=pjActionSaveLocale").done(function (data) {
					if (data && data.status && data.status === "OK" && data.id) {
						...
						$grid.datagrid("load", "admin.php?controller=pjLocale&action=pjActionGetLocale");
					}
				});
				return false;
				...
				$.post("admin.php?controller=pjLocale&action=pjActionSaveDefault", {
