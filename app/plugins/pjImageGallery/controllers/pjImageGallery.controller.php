...
class pjImageGallery extends pjImageGalleryAppController
{
	public $pjImageGalleryKey = 'pjImageGalleryKey';
	
	private $pjImageGalleryError = 'pjImageGalleryError';
	...
	public function pjActionIsFlagReady()
	{
		...
		$cnt = pjImageGalleryModel::factory()
			->join('pjImageGalleryLanguage', 't2.iso=t1.language_iso', 'left')
	...
	public function pjActionLocales()
	{
		...
		{
			$arr = pjImageGalleryLanguageModel::factory()->where('t1.file IS NOT NULL')->orderBy('t1.title ASC')->findAll()->getData();
			...
			$this->appendJs('pjImageGallery.js', $this->getConst('PLUGIN_JS_PATH'));
	...
	public function pjActionSaveFields()
	{
		...
		pjUtil::redirect(sprintf("%sindex.php?controller=pjImageGallery&action=%s&err=PAL01&tab=1&q=%s&locale=%u&page=%u", PJ_INSTALL_URL, $_POST['next_action'], urlencode($_POST['q']), $_POST['locale'], $_POST['page']));
		exit;
	}
	
	private function pjActionCheckDefault()
	{
		if (0 == pjImageGalleryModel::factory()->where('is_default', 1)->findCount()->getData())
		{
			pjImageGalleryModel::factory()->limit(1)->modifyAll(array('is_default' => 1));
		}
	}
	...
	public function pjActionDeleteLocale()
	{
		...
		if ($this->isXHR())
		{
			...
			if ($pjImageGalleryModel->reset()->set('id', $_GET['id'])->erase()->getAffectedRows() != 1)
			{
	...
	public function pjActionImportExport()
	{
    	...
    	$pjImageGalleryModel = pjImageGalleryModel::factory()
			->select('t1.*, t2.title')
			->join('pjImageGalleryLanguage', 't2.iso=t1.language_iso');
      
    	if (!isset($this->option_arr['o_multi_lang']) || (int) $this->option_arr['o_multi_lang'] === 0)
		{
			$pjImageGalleryModel->where('t1.is_default', 1);
		}
    
		$this->set('locale_arr',$pjImageGalleryModel->orderBy('t1.sort ASC')
			->findAll()
			->getData()
		);
	}
	...
	public function pjActionImportConfirm()
	{
		...
		$locale_arr = $pjImageGalleryModel->orderBy('t1.sort ASC')
			->findAll()
			->getDataPair('id');
		...
		if (isset($_POST['import']))
		{
			if (isset($_FILES['file'], $_POST['separator']))
			{
				...
				if ($pjUpload->load($_FILES['file']))
				{
					if (($handle = fopen($pjUpload->getFile('tmp_name'), "rb")) !== FALSE)
					{
						...
						$time = time();
						if (!isset($_SESSION[$this->pjImageGalleryError]))
						{
							$_SESSION[$this->pjImageGalleryError] = array();
						}
						...
						while (($data = fgetcsv($handle, 0, $separator)) !== FALSE)
						{
							if (!empty($data))
							{
								...
								if (!empty($nl))
								{
									$_SESSION[$this->pjImageGalleryError][$time] = sprintf(__('plugin_locale_error_line', true), $i);
									$err = 'PAL14&tm=' . $time;
									break;
								}
			...
			pjUtil::redirect(PJ_INSTALL_URL . "index.php?controller=pjImageGallery&action=pjActionImportConfirm&tab=1&err=".$err);
		}
		
		$this->set('locale_arr', $locale_arr);
		
		if (isset($_GET['tm'], $_SESSION[$this->pjImageGalleryError][$_GET['tm']]))
		{
			$this->set('tm_text', $_SESSION[$this->pjImageGalleryError][$_GET['tm']]);
		}
	}
	...
	public function pjActionImport()
	{
		...
		if (isset($_POST['import']) && $this->isLoged() && $this->isAdmin())
		{
				...
				&& !empty($_SESSION[$_POST['key']]['separator']))
			{
				...
				{
					$locale_pairs = pjImageGalleryModel::factory()
		...
		pjUtil::redirect(PJ_INSTALL_URL . "index.php?controller=pjImageGallery&action=pjActionImportExport&tab=1&err=".$err);
	}
	...
	public function pjActionIndex()
	{
		...
		$locale_arr = $pjImageGalleryModel->orderBy("is_default DESC, t1.id ASC")->findAll()->getData();
		...
		foreach ($_arr as $_k => $_v)
		{
			$pjMultiLangModel->reset()
				...
				->join('pjImageGallery', 't2.id=t1.locale', 'left')
				->where('model', 'pjField')
				->where('field', 'title')
	...
	public function pjActionSaveLocale()
	{
		...
		if ($this->isXHR())
		{
				...
				&& (int) $_GET['id'] > 0)
			{
				...
				{
					$tmp = pjImageGalleryLanguageModel::factory()->where('t1.iso', $_POST['value'])->limit(1)->findAll()->getDataIndex(0);
				...
				pjImageGalleryModel::factory()->set('id', $_GET['id'])->modify($data);
				pjAppController::jsonResponse(array('status' => 'OK', 'code' => 201, 'text' => 'Locale has been updated.'));
			}
			
			$lang = pjImageGalleryLanguageModel::factory()
				->where(sprintf("t1.iso NOT IN (SELECT `language_iso` FROM `%s`)", pjImageGalleryModel::factory()->getTable()))
	...
	{
		$language = pjImageGalleryLanguageModel::factory()->where('t1.iso', $iso)->limit(1)->findAll()->getDataIndex(0);
		
		$pjImageGalleryModel = pjImageGalleryModel::factory();
		
		$pjImageGalleryModel->begin();
		
		$statement = sprintf("SET @sort := (SELECT MAX(`sort`) + 1 FROM `%s` LIMIT 1);", $pjImageGalleryModel->getTable());
		$pjImageGalleryModel->prepare($statement)->exec();
		
		$statement = sprintf("INSERT IGNORE INTO `%s` (`id`, `language_iso`, `name`, `dir`, `sort`, `is_default`) VALUES (NULL, :language_iso, :name, :dir, @sort, 0);", $pjImageGalleryModel->getTable());
		$insert_id = $pjImageGalleryModel
			->prepare($statement)
			->exec(array(
				'language_iso' => $iso,
		...
		if (empty($locale_id))
		{
			$arr = $pjImageGalleryModel->reset()->findAll()->getData();
	...
	public function pjActionSaveDefault()
	{
		...
		if ($this->isXHR())
		{
			$affected_rows = pjImageGalleryModel::factory()
				->where(1,1)
	...
	public function pjActionSortLocale()
	{
		...
		if ($this->isXHR())
		{
			$LocaleModel = new pjImageGalleryModel();
	...
	public function pjActionClean()
	{
		...
		if (isset($_POST['clean_step']))
		{
			...
			pjUtil::redirect($_SERVER['PHP_SELF'] . "?controller=pjImageGallery&action=pjActionClean");
		}
	...
	public function pjActionFlagReset()
	{
		...
		if ($this->isXHR())
		{
			...
			$affected_rows = $pjImageGalleryModel
				->reset()
				->set('id', $_POST['id'])
				->modify(array(
					'flag' => ':NULL'
