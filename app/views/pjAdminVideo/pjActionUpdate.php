...
if (isset($tpl['status']))
{
	...
	<div class="ui-tabs ui-widget ui-widget-content ui-corner-all b10">
		<ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-all">
			<li class="ui-state-default ui-corner-top"><a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminVideo&amp;action=pjActionIndex"><?php __('menuVideo'); ?></a></li>
		</ul>
	</div>
	
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminVideo&amp;action=pjActionUpdate" method="post" id="frmUpdateVideo" class="pj-form form" enctype="multipart/form-data">
		<input type="hidden" name="image_video_update" value="1" />
		<input type="hidden" id="index_arr" name="index_arr" value="" />
		<input type="hidden" id="remove_arr" name="remove_arr" value="" />
		<input type="hidden" name="id" value="<?php echo $tpl['arr']['id']?>" />
		<?php
		pjUtil::printNotice(__('infoEditVideo', true), __('infoEditVideoDesc', true)); 
		?>
		...
		<?php
			...
			?>
			<p>
				<label class="title"><?php __('lblVideo'); ?></label>
				<span class="inline_block">
					<input type="file" name="video_path" id="video_path" class="pj-form-field w300" />
				</span>
			</p>
			<?php
			if(!empty($tpl['arr']['video_path']))
			{
				$image_url = PJ_INSTALL_URL . $tpl['arr']['video_path'];
				$mime_type = $tpl['arr']['mime_type'];
				?>
				<p id="image_container">
					<label class="title">&nbsp;</label>
					<span class="inline_block">
					<video width="320" height="240" controls>
					<source src="<?php echo $image_url; ?>" type="<?php echo $mime_type; ?>">
					</video> 
					</span>
				</p>
				<?php
		...
		<p>
			...
			<input type="button" value="<?php __('btnCancel'); ?>" class="pj-button" onclick="window.location.href='<?php echo PJ_INSTALL_URL; ?>admin.php?controller=pjAdminVideo&action=pjActionIndex';" />
		</p>
	</form>
