...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminSponsors&amp;action=pjActionUpdate" method="post" id="frmUpdateSponsor" class="pj-form form" enctype="multipart/form-data">
		<input type="hidden" name="image_sponsor_update" value="1" />
		<input type="hidden" id="index_arr" name="index_arr" value="" />
		<input type="hidden" id="remove_arr" name="remove_arr" value="" />
		<input type="hidden" name="id" value="<?php echo $tpl['arr']['id']?>" />
		<?php
		pjUtil::printNotice(__('infoEditSponsor', true), __('infoEditSponsorDesc', true)); 
		?>
		...
		<div class="clear_both">
			...
			<p>
				...
				<span class="inline_block">
					<input type="file" name="sponsor_image" id="sponsor_image" class="pj-form-field w300" />
				</span>
			</p>
			<?php
			if(!empty($tpl['arr']['sponsor_image']))
			{
				$image_url = PJ_INSTALL_URL . $tpl['arr']['sponsor_image'];
				?>
				<p id="image_container">
					<label class="title">&nbsp;</label>
					<span class="inline_block">
						<img class="td-image" src="<?php echo $image_url; ?>" style="width:263px"/>
						<!-- <a href="javascript:void(0);" class="pj-delete-image" data-href="<?php //echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminSponsors&amp;action=pjActionDeleteImage&id=<?php //echo $tpl['arr']['id'];?>"><?php //__('btnDelete');?></a> -->
					</span>
				</p>
				<?php
			} 
			...
			<p>
				...
				<span class="inline_block">
					<input type="text" name="sponsor_link" id="sponsor_link" class="pj-form-field w300 required" value="<?php echo htmlspecialchars(stripslashes(@$tpl['arr']['sponsor_link'])); ?>" />
				</span>
			</p>
