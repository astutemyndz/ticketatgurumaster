...
if (isset($tpl['status']))
{
	...
	<form action="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjAdminSlider&amp;action=pjActionCreate" method="post" id="frmCreateSlider" class="pj-form form" enctype="multipart/form-data">
		<input type="hidden" name="image_slider_create" value="1" />
		<input type="hidden" id="index_arr" name="index_arr" value="" />
		<input type="hidden" id="remove_arr" name="remove_arr" value="" />
		<?php
		pjUtil::printNotice(__('infoAddSlider', true), __('infoAddSliderDesc', true)); 
		?>
		...
		<?php
			...
			<p>
				...
				<span class="inline_block">
					<input type="file" name="slider_image" id="slider_image" class="pj-form-field w300 required" />
				</span>
			</p>
			<p>
				<label class="title"><?php __('cmsBannerLink'); ?></label>
				<span class="inline_block">
					<input type="text" name="slider_link" id="slider_link" class="pj-form-field w300 required" />
				</span>
			</p>		
		</div>
