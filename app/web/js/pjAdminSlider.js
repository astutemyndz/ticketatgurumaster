...
(function ($, undefined) {
	$(function () {
		...
		if ($frmUpdateShow.length > 0 && validate) {
			...
			$frmUpdateShow.validate({
				...
				submitHandler: function(form)
				{
					...
					{
						$.post("admin.php?controller=pjAdminSlider&action=pjActionBeforeSave", post_arr[i], callback);
					}
					function callback(data){
						...
						if(loop < post_arr.length)
						{
							...
							$.post("admin.php?controller=pjAdminSlider&action=pjActionSave").done(function (data) {
								...
								if(data.code == '101')
								{
									...
									$dialogShowStatus.dialog("option", "close", function () {
							    		...
							    		if(data.code == '200')
							    		{
							    			window.location.href="admin.php?controller=pjAdminSlider&action=pjActionShow&id="+data.id+"&err=" + data.text;
							    		}
							    	});
									$dialogShowStatus.dialog("option", "buttons", {
							    		'Close': function () {
		...
		if ($frmCreateSlider.length > 0 && validate) {
			$frmCreateSlider.validate({
				...
				submitHandler: function(form){
					...
					$("#frmCreateSlider .fdRequired").each(function() {
						if($(this).val() == '')
						{
							valid = false;
					    	$(this).addClass('pj-error-field');
		...
		if ($frmUpdateSlider.length > 0 && validate) {
			...
			$dialogDeleteShow.dialog({
				...
				buttons: (function () {
					...
					buttons[tbApp.locale.button.delete] = function () {
						...
						$.post("admin.php?controller=pjAdminSlider&action=pjActionDeleteShow", {
							"id": id
						}).done(function (data) {
		...
		function formatImage (str, obj) {
			...
			return ['<a href="admin.php?controller=pjAdminSlider&action=pjActionUpdate&id=', obj.id ,'"><img src="', src, '" style="width: 80px; display:block;" /></a>'].join("");
		}
		if ($("#grid").length > 0 && datagrid) {
			//console.log(datagrid);
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjAdminSlider&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "admin.php?controller=pjAdminSlider&action=pjActionDeleteSlider&id={:id}", beforeShow: onBeforeShow},
				          ],
				columns: [
				...
				dataUrl: "admin.php?controller=pjAdminSlider&action=pjActionGetSlider",
				dataType: "json",
				fields: ['slider_image', 'name', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "admin.php?controller=pjAdminSlider&action=pjActionDeleteSliderBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "admin.php?controller=pjAdminSlider&action=pjActionSaveSlider&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjAdminSlider&action=pjActionGetSlider", "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$.post("admin.php?controller=pjAdminSlider&action=pjActionSetActive", {
				id: $(this).closest("tr").data("object")['id']
			}).done(function (data) {
				$grid.datagrid("load", "admin.php?controller=pjAdminSlider&action=pjActionGetSlider");
			});
			return false;
			...
			if(date_time != '')
			{
				...
				$.post("admin.php?controller=pjAdminSlider&action=pjActionAddShow&event_id="+event_id+"&period=" + $(this).attr('data-period'), post_data).done(function (data) {
