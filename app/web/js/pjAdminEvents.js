...
(function ($, undefined) {
	...
	let viewGarbageRecord;
	let overwriteURL;
	let overwriteDeleteQueryString;
	let overwriteBulkDeleteQueryString;
	let overwriteViewQueryString;
	let queryString;
	let overwriteDataUrlQueryString;
	let overwriteDeleteShowUrlQueryString;

	const setViewGarbageRecord = function(value) {
		viewGarbageRecord = value;
		return this;
	}
	const setQueryString = function(value) {
		queryString = value;
		return queryString;
	}

	// show
	const setOverwriteDeleteShowUrlQueryString = function(value) {
		overwriteDeleteShowUrlQueryString = value;
		return overwriteDeleteShowUrlQueryString;
	}
	// Delete garbage data
	const setOverwriteDeleteQueryString = function(value) {
		overwriteDeleteQueryString = setQueryString(value);
		return overwriteDeleteQueryString;
	}

	// Delete bulk garbage data
	const setOverwriteBulkDeleteQueryString = function(value) {
		overwriteBulkDeleteQueryString = setQueryString(value);
		return overwriteBulkDeleteQueryString;
	}
	

	const setOverwriteViewQueryString = function(value) {
		overwriteViewQueryString = setQueryString(value);
		return overwriteViewQueryString;
	}
	const setOverwriteDataUrlQueryString = function(value) {
		overwriteDataUrlQueryString = setQueryString(value);
		return overwriteDataUrlQueryString;
	}
	if(urlParams.has('viewGarbageRecord') && urlParams.get('viewGarbageRecord') != undefined && parseInt(urlParams.get('viewGarbageRecord')) === 1) {
		setViewGarbageRecord(urlParams.get('viewGarbageRecord'));
	}
	
	if(viewGarbageRecord) {
		setOverwriteViewQueryString('?controller=pjAdminEvents&action=pjActionGetEvent&viewGarbageRecord=1');
		setOverwriteDeleteQueryString('?controller=pjAdminEvents&action=pjActionDeleteEvent&id={:id}&deleteGarbageRecord=1');
		setOverwriteBulkDeleteQueryString('?controller=pjAdminEvents&action=pjActionDeleteEventBulk&viewGarbageRecord=1&deleteBulkGarbageRecord=1');
		setOverwriteDataUrlQueryString('?controller=pjAdminEvents&action=pjActionSaveEvent&viewGarbageRecord=1&id={:id}');
		setOverwriteDeleteShowUrlQueryString('?controller=pjAdminEvents&action=pjActionDeleteShow&viewGarbageRecord=1');
	} else {
		setOverwriteViewQueryString('?controller=pjAdminEvents&action=pjActionGetEvent');
		setOverwriteDeleteQueryString('?controller=pjAdminEvents&action=pjActionDeleteEvent&id={:id}');
		setOverwriteBulkDeleteQueryString('?controller=pjAdminEvents&action=pjActionDeleteEventBulk');
		setOverwriteDataUrlQueryString('?controller=pjAdminEvents&action=pjActionSaveEvent&id={:id}');
		setOverwriteDeleteShowUrlQueryString('?controller=pjAdminEvents&action=pjActionDeleteShow');
	}
	$(function () {
		"use strict";
			...
			$dialogDeleteShow.dialog({
				...
				buttons: (function () {
					...
					buttons[tbApp.locale.button.delete] = function () {
						...
						$.post(overwriteDeleteShowUrlQueryString, {
							"id": id
						}).done(function (data) {
		...
		if ($("#grid").length > 0 && datagrid) {
			...
			//console.log(overwriteDeleteQueryString);
			//console.log(overwriteViewQueryString);
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjAdminEvents&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: overwriteDeleteQueryString, beforeShow: onBeforeShow},
				...
				columns: [{text: myLabel.image, type: "text", sortable: false, editable: false, width: 85, renderer: formatImage},
				          ...
				          {text: (viewGarbageRecord) ? 'Restore' : myLabel.status, type: "select", sortable: true, editable: true, width: 90, options: [
				                                                                                     {label: (viewGarbageRecord) ? 'TRUE' : myLabel.active, value: "T"}, 
				                                                                                     {label: (viewGarbageRecord) ? 'FALSE' : myLabel.inactive, value: "F"}
				                                                                                     ], applyClass: "pj-status"}],
				dataUrl: overwriteViewQueryString,
				dataType: "json",
				fields: ['event_img', 'title', 'duration', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: overwriteBulkDeleteQueryString, render: true, confirmation: myLabel.delete_confirmation},
				...
				saveUrl: overwriteDataUrlQueryString,
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", overwriteViewQueryString, "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$.post("admin.php?controller=pjAdminEvents&action=pjActionSetActive", {
				...
				$grid.datagrid("load", overwriteViewQueryString);
			});
			return false;
