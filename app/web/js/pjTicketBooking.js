	...
	TicketBooking.prototype = {
		...
		init: function (opts) {
			...
			this.$container.on("click.tb", ".tbSelectorLocale", function (e) {
				...
				pjQ.$.get([self.opts.folder, "admin.php?controller=pjFront&action=pjActionLocale"].join(""), {
						...
						dOpts = {
							...
							onClose: function(dateText)
							{
								if(pjQ.$(this).attr('data-list') == '1')
								{
									...
									pjQ.$.get([self.opts.folder, "admin.php?controller=pjFront&action=pjActionGetTime"].join(""), {"session_id": self.opts.session_id, "id" : pjQ.$(this).attr('data-id'), "date": dateText}).done(function (data) {
				...
				{
					self.disableButtons.call(self);
					pjQ.$.post([self.opts.folder, "admin.php?controller=pjFront&action=pjActionSaveDateTime", "&session_id=", self.opts.session_id].join(""), $frm.serialize()).done(function (data) {
						if(data.code == '200')
						{
				...
				if(!pjQ.$(this).hasClass('pjCbTimePassed'))
				{
					...
					self.from_date = pjQ.$(this).attr('data-from_date');
					pjQ.$.post([self.opts.folder, "admin.php?controller=pjFront&action=pjActionSaveDateTime", "&session_id=", self.opts.session_id].join(""), params).done(function (data) {
						if(data.code == '200')
						{
							hashBang("#!/Seats/date:" + date);
						}
				...
				if(total_seats == 0)
				{
					...
					self.disableButtons.call(self);
					pjQ.$.post([self.opts.folder, "admin.php?controller=pjFront&action=pjActionSaveSeats", "&session_id=", self.opts.session_id].join(""), params).done(function (data) {
						if(data.code == '200')
						{
				...
				self.disableButtons.call(self);
				pjQ.$.get([self.opts.folder, "admin.php?controller=pjFront&action=pjActionSetVenue", "&session_id=", self.opts.session_id, "&venue_id=", pjQ.$(this).val()].join("")).done(function (data) {
		...
		loadEvents: function () {
			...
			pjQ.$.get([this.opts.folder, "admin.php?controller=pjFront&action=pjActionEvents", "&session_id=", self.opts.session_id].join(""), params).done(function (data) {
		...
		loadEventDetails: function () {
			...
			pjQ.$.get([this.opts.folder, "admin.php?controller=pjFront&action=pjActionDetails", "&session_id=", self.opts.session_id].join(""), params).done(function (data) {
				self.$container.html(data);
				pjQ.$('html, body').animate({
			        scrollTop: self.$container.offset().top
			    }, 500);
		...
		loadSeats: function () {
			...
			pjQ.$.get([this.opts.folder, "admin.php?controller=pjFront&action=pjActionSeats", "&session_id=", self.opts.session_id].join(""), params).done(function (data) {
				self.$container.html(data);
				pjQ.$('html, body').animate({
			        scrollTop: self.$container.offset().top
			    }, 500);
				...
				{
					pjQ.$('[data-toggle="tooltip"]').tooltip({container:'#pjWrapperTicketBooking_' + self.opts.layout});
				}
			}).fail(function () {
				self.enableButtons.call(self);
			});
		...
		loadCheckout: function () {
			...
			pjQ.$.get([this.opts.folder, "admin.php?controller=pjFront&action=pjActionCheckout", "&session_id=", self.opts.session_id].join(""), params).done(function (data) {
				self.$container.html(data);
				pjQ.$('html, body').animate({
			        scrollTop: self.$container.offset().top
			    }, 500);
				...
				if (validate) 
				{
					pjQ.$('#frmCheckoutForm_'+ self.opts.index).validate({
						...
						submitHandler: function (form) {
							self.disableButtons.call(self);
							var $form = pjQ.$(form);
							pjQ.$.post([self.opts.folder, "admin.php?controller=pjFront&action=pjActionCheckout", "&session_id=", self.opts.session_id].join(""), $form.serialize()).done(function (data) {
								if (data.status == "OK") {
		...
		loadPreview: function () {
			...
			pjQ.$.get([this.opts.folder, "admin.php?controller=pjFront&action=pjActionPreview", "&session_id=", self.opts.session_id].join(""), params).done(function (data) {
				self.$container.html(data);
				pjQ.$('html, body').animate({
			        scrollTop: self.$container.offset().top
			    }, 500);
				if (validate) {
					pjQ.$('#frmPreviewForm_'+ self.opts.index).validate({
						...
						submitHandler: function (form) {
							self.disableButtons.call(self);
							var $form = pjQ.$(form);
							pjQ.$.post([self.opts.folder, "admin.php?controller=pjFront&action=pjActionSaveBooking", "&session_id=", self.opts.session_id].join(""), $form.serialize()).done(function (data) {
								if (data.code == "200") {
		...
		getPaymentForm: function(obj){
			...
			pjQ.$.get([this.opts.folder, "admin.php?controller=pjFront&action=pjActionGetPaymentForm", "&session_id=", self.opts.session_id].join(""), qs).done(function (data) {
				self.$container.html(data);
				pjQ.$('html, body').animate({
			        scrollTop: self.$container.offset().top
			    }, 500);
