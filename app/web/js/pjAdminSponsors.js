...
(function ($, undefined) {
	$(function () {
		"use strict";
		var $frmCreateSponsor = $("#frmCreateSponsor"),
			$frmUpdateSponsor = $("#frmUpdateSponsor"),
		...
		if ($frmCreateSponsor.length > 0 || $frmUpdateSponsor.length > 0) {
		...
		if ($frmCreateSponsor.length > 0 && validate) {
			$frmCreateSponsor.validate({
				rules:{
					"duration": {
						positiveNumber: true
					}
				...
				submitHandler: function(form){
					...
					$("#frmCreateSponsor .fdRequired").each(function() {
						if($(this).val() == '')
						{
							valid = false;
		...
		if ($frmUpdateSponsor.length > 0 && validate) {
			$frmUpdateSponsor.validate({
				rules:{
					"duration": {
						positiveNumber: true
					}
		...
		function formatImage (str, obj) {
			...
			return ['<a href="admin.php?controller=pjAdminSponsors&action=pjActionUpdate&id=', obj.id ,'"><img src="', src, '" style="width: 80px; display:block;" /></a>'].join("");
		}
		if ($("#grid").length > 0 && datagrid) {
			//console.log(datagrid);
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjAdminSponsors&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "admin.php?controller=pjAdminSponsors&action=pjActionDeleteSponsor&id={:id}", beforeShow: onBeforeShow},
				          ],
				columns: [
				    ...
				    {text: myLabel.year, type: "text", sortable: true, editable: false, width: 80},
				...
				dataUrl: "admin.php?controller=pjAdminSponsors&action=pjActionGetSponsors",
				dataType: "json",
				fields: ['sponsor_image', 'name', 'sponsor_year', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "admin.php?controller=pjAdminSponsors&action=pjActionDeleteSponsorBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "admin.php?controller=pjAdminSponsors&action=pjActionSaveSponsor&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjAdminSponsors&action=pjActionGetSponsors", "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$.post("admin.php?controller=pjAdminSponsors&action=pjActionSetActive", {
				id: $(this).closest("tr").data("object")['id']
			}).done(function (data) {
				$grid.datagrid("load", "admin.php?controller=pjAdminSponsors&action=pjActionGetSponsors");
			});
			return false;
