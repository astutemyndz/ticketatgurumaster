...
(function ($, undefined) {
	$(function () {
		...
		function formatImage (str, obj) {
			...
			return ['<a href="admin.php?controller=pjAdminArtists&action=pjActionUpdate&id=', obj.id ,'"><img src="', src, '" style="width: 80px; display:block;" /></a>'].join("");
		}
		if ($("#grid").length > 0 && datagrid) {
			//console.log(datagrid);
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjAdminArtists&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "admin.php?controller=pjAdminArtists&action=pjActionDeleteArtist&id={:id}", beforeShow: onBeforeShow},
				          ],
				columns: [
				...
				dataUrl: "admin.php?controller=pjAdminArtists&action=pjActionGetArtists",
				dataType: "json",
				fields: ['artist_image', 'name', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "admin.php?controller=pjAdminArtists&action=pjActionDeleteArtistBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "admin.php?controller=pjAdminArtists&action=pjActionSaveArtist&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjAdminArtists&action=pjActionGetArtists", "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$.post("admin.php?controller=pjAdminArtists&action=pjActionSetActive", {
				...
				$grid.datagrid("load", "admin.php?controller=pjAdminArtists&action=pjActionGetArtists");
			});
			return false;
