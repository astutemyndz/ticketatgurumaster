...
(function ($, undefined) {
	$(function () {
		...
		function formatImage (str, obj) {
			...
			return ['<img src="', src, '" style="width: 80px; display:block;" />'].join("");
		}
			...
			var $grid = $("#grid").datagrid({
				...
				columns: [
					...
					{text: myLabel.status, type: "select", sortable: true, editable: true, width: 90, 
						options: [
				                    {label: myLabel.true, value: "T"}, 
				            		{label: myLabel.false, value: "F"}
								], applyClass: "pj-status"}
				],
				dataUrl: "admin.php?controller=pjAdminArtists&action=pjActionDeletedArtistsAjax",
				dataType: "json",
				...
				saveUrl: "admin.php?controller=pjAdminArtists&action=pjActionRestoreArtist&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjAdminArtists&action=pjActionDeletedArtistsAjax", "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$.post("admin.php?controller=pjAdminArtists&action=pjActionRestoreArtist", {
				id: $(this).closest("tr").data("object")['id']
			}).done(function (data) {
                console.log(data);
                
				$grid.datagrid("load", "admin.php?controller=pjAdminArtists&action=pjActionDeletedArtistsAjax");
			});
			return false;
