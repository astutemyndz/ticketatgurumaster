	...
	$(function () {
			...
			$frmCreateSubscriber.validate({
				rules: {
					"email": {
						required: true,
						email: true,
						remote: "admin.php?controller=pjAdminSubscribers&action=pjActionCheckEmail"
					}
				},
				messages: {
					"group_id[]":{
			...
			$frmUpdateSubscriber.validate({
				rules: {
					"email": {
						required: true,
						email: true,
						remote: "admin.php?controller=pjAdminSubscribers&action=pjActionCheckEmail&id=" + $frmUpdateSubscriber.find("input[name='id']").val()
					}
				},
				messages: {
					"group_id[]":{
		...
		if ($("#grid").length > 0 && datagrid) 
		{
			...
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjAdminSubscribers&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "admin.php?controller=pjAdminSubscribers&action=pjActionDeleteSubscriber&id={:id}", beforeShow: onBeforeShow},
				          {type: "send", url: "admin.php?controller=pjAdminSubscribers&action=pjActionSend&id={:id}"}],
				...
				columns: [{text: myLabel.name_email, type: "text", sortable: false, editable: false, width: 140},
				          ...
				          {text: myLabel.status, type: "select", sortable: true, editable: true, width: 110, options: [
				                                                                                     ...
				                                                                                     ], applyClass: "pj-subscribed"}],
				dataUrl: "admin.php?controller=pjAdminSubscribers&action=pjActionGetSubscriber" + pjGrid.queryString,
				dataType: "json",
				fields: ['name_email', 'groups', 'total_sent', 'last_sent', 'subscribed'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "admin.php?controller=pjAdminSubscribers&action=pjActionDeleteSubscriberBulk", render: true, confirmation: myLabel.delete_confirmation},
					   {text: myLabel.exported_selected, url: "admin.php?controller=pjAdminSubscribers&action=pjActionExportSubscriber", ajax: false},
				...
				saveUrl: "admin.php?controller=pjAdminSubscribers&action=pjActionSaveSubscriber&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjAdminSubscribers&action=pjActionGetSubscriber", "first_name", "ASC", content.page, content.rowCount);
			return false;
			...
			$.post("admin.php?controller=pjAdminSubscribers&action=pjActionSetActive", {
				id: $(this).closest("tr").data("object")['id']
			}).done(function (data) {
				$grid.datagrid("load", "admin.php?controller=pjAdminSubscribers&action=pjActionGetSubscriber");
			});
			return false;
		...
		$("#grid").on("click", '.pj-paginator-action', function (e) {
			if($(this).html() == myLabel.add_to_group)
			{
				e.preventDefault();
				var subscriber_id = $('.pj-table-select-row:checked').map(function(e){
					 return $(this).val();
				}).get();
				if(subscriber_id != '' && subscriber_id != null)
				{
							...
							'Assign': function() {
								...
								if(group_id != '' && group_id != null)
								{
									$.ajax({
										...
										dataType: 'json',
										url: "admin.php?controller=pjAdminSubscribers&action=pjActionAddToGroup",
										success: function (res) {
											if(res.code == 200)
											{
												$grid.datagrid("load", "admin.php?controller=pjAdminSubscribers&action=pjActionGetSubscriber");
					...
					$(".pj-menu-list-wrap").css('display', 'none');
				}	
				return false;
			}//add to group
							...
							'Send': function() {
								if($('#message_id').val() != '')
								{
									...
									$.ajax({
										...
										dataType: 'json',
										url: "admin.php?controller=pjAdminSubscribers&action=pjActionSend&id=" + subscriber_id,
										success: function (res) {
											if(res.code == 200)
											{
												$('#result_message').html(myLabel.sent_ok);
