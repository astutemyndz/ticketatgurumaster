...
(function ($, undefined) {
	$(function () {
		"use strict";
		var $frmCreateVideo = $("#frmCreateVideo"),
			$frmUpdateVideo = $("#frmUpdateVideo"),
		...
		if ($frmCreateVideo.length > 0 || $frmUpdateVideo.length > 0) {
		...
		if ($frmCreateVideo.length > 0 && validate) {
			tinymce.init({
			...
			$frmCreateVideo.validate({
				...
				submitHandler: function(form){
					...
					$("#frmCreateVideo .fdRequired").each(function() {
						if($(this).val() == '')
						{
							valid = false;
		...
		if ($frmUpdateVideo.length > 0 && validate) {
			tinymce.init({
			    selector: "textarea.mceEditor",
			    theme: "modern",
			    relative_urls : false,
		...
		function formatImage (str, obj) {
			...
			return ['<a href="admin.php?controller=pjAdminVideo&action=pjActionUpdate&id=', obj.id ,'"><video width="200" height="150" controls><source src="', src, '" type="',obj.mime_type,'"></video></a>'].join("");
		}
		if ($("#grid").length > 0 && datagrid) {
			//console.log(datagrid);
			var $grid = $("#grid").datagrid({
				buttons: [{type: "edit", url: "admin.php?controller=pjAdminVideo&action=pjActionUpdate&id={:id}"},
				          {type: "delete", url: "admin.php?controller=pjAdminVideo&action=pjActionDeleteVideo&id={:id}", beforeShow: onBeforeShow},
				          ],
				columns: [
					{text: myLabel.image, type: "text", sortable: false, editable: false, width: 100, renderer: formatImage},
					{text: myLabel.mime_type, type: "text", sortable: true, editable: false, width: 100},
					{text: myLabel.title, type: "text", sortable: true, editable: false, width: 200},
				...
				dataUrl: "admin.php?controller=pjAdminVideo&action=pjActionGetVideo",
				dataType: "json",
				fields: ['video_path','mime_type', 'name', 'status'],
				paginator: {
					actions: [
					   {text: myLabel.delete_selected, url: "admin.php?controller=pjAdminVideo&action=pjActionDeleteVideoBulk", render: true, confirmation: myLabel.delete_confirmation}
					],
					gotoPage: true,
					paginate: true,
					total: true,
				...
				saveUrl: "admin.php?controller=pjAdminVideo&action=pjActionSaveVideo&id={:id}",
				select: {
					field: "id",
					name: "record[]"
				}
		...
		$(document).on("click", ".btn-all", function (e) {
			...
			$grid.datagrid("load", "admin.php?controller=pjAdminVideo&action=pjActionGetVideo", "created", "DESC", content.page, content.rowCount);
			return false;
			...
			$.post("admin.php?controller=pjAdminVideo&action=pjActionSetActive", {
				id: $(this).closest("tr").data("object")['id']
			}).done(function (data) {
				$grid.datagrid("load", "admin.php?controller=pjAdminVideo&action=pjActionGetVideo");
			});
			return false;
