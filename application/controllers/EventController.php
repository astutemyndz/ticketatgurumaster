	...
	*/
	...
	public function index() {
		...
		*/
        ...
        $this->load->view('frontend/pages/event/index');
	...
	private function pjActionEvents()
	{
		...
		$arr = $pjEventModel
			...
			->join('pjMultiLang', "t3.model='pjEvent' AND t3.foreign_id=t1.id AND t3.field='small_description' AND t3.locale='".$this->getLocaleId()."'", 'left outer')
	...
	private function pjGetEvent($id) {
		...
		$arr = $pjEventModel
				...
				->select('t1.*, t2.content as title, t4.content as small_description, t3.content as description')
				->find($id)
				->getData();
			return $arr;
	}
	...
	public function show() {
		...
		$eventSlug = ($this->uri->segment(2)) ? $this->uri->segment(2) : '';
		$event = pjEventModel::factory()
						->join('pjMultiLang', "t2.model='pjEvent' AND t2.foreign_id=t1.id AND t2.field='title' AND t2.locale='".$this->getLocaleId()."'", 'left outer')
						->join('pjMultiLang', "t3.model='pjEvent' AND t3.foreign_id=t1.id AND t3.field='description' AND t3.locale='".$this->getLocaleId()."'", 'left outer')
						->join('pjMultiLang', "t4.model='pjEvent' AND t4.foreign_id=t1.id AND t4.field='small_description' AND t4.locale='".$this->getLocaleId()."'", 'left outer')
						->select('t1.id')
						->where('t1.slug', $eventSlug)
						->findAll()
						->limit(1)
						->getData();
		if(!empty($event) && is_array($event)) {
			$this->setId($event[0]['id']);
		}
		if($this->id) {
			$this->data['event'] = $this->pjGetEvent($this->getId());
		}
		$artists = pjAssignArtistToEventModel::factory()
		...
		///redirect('/404');
		// echo "<pre>";
		// print_r($this->data['artists']);
		// exit;
		$this->load->view('frontend/layout/head');
		$this->load->view('frontend/layout/inner_page_header');
		$this->load->view('frontend/pages/event/show', $this->data);
		$this->load->view('frontend/layout/footer');
	}
	public function search() {
		$this->load->view('frontend/layout/head');
        $this->load->view('frontend/layout/inner_page_header');
        $this->load->view('frontend/pages/event/search');
		$this->load->view('frontend/layout/footer');
	}
